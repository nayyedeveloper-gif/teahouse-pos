            ...
            <div class="p-6">
                ...
                <div class="space-y-2 mb-4">
                    ...
                    <div class="flex items-center text-sm">
                        ...
                        <span class="ml-2 font-medium myanmar-text">{{ $order->table->name_mm }}</span>
                    </div>
                    @endif
                    ...
                    <div class="flex items-center text-sm">
                        ...
                        <span class="ml-2 font-medium">{{ $order->orderItems->count() }} <span class="myanmar-text">မျိုး</span></span>
                    </div>
                    ...
                    <div class="flex items-center text-sm">
                        ...
                        <span class="ml-2 font-medium">{{ $order->created_at->format('H:i') }}</span>
                    </div>
                    ...
                    <div class="flex items-center text-sm">
                        ...
                        <span class="ml-2 font-medium">{{ $order->waiter->name }}</span>
                    </div>
                    @endif
                </div>
                ...
                <div class="pt-4 border-t border-gray-200">
                    <div class="flex justify-between items-center mb-3">
                        ...
                        <span class="text-lg font-bold text-gray-900">{{ number_format($order->total, 0) }} Ks</span>
                    </div>
                    
                    @if($order->status === 'pending')
                    <button 
                        wire:click.stop="openPaymentModal({{ $order->id }})"
            ...
            <div class="p-6">
                ...
                <div class="mb-6 pb-4 border-b">
                    <div class="flex justify-between items-start">
                        <div>
                            <h4 class="text-lg font-bold text-gray-900">{{ $paymentOrder->order_number }}</h4>
                            @if($paymentOrder->table)
                            <p class="text-sm text-gray-600 myanmar-text">{{ $paymentOrder->table->name_mm }}</p>
                            @endif
                        </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="space-y-2">
                        @foreach($paymentOrder->orderItems as $item)
                        <div class="flex justify-between text-sm">
                            <div class="flex-1">
                                <span class="text-gray-900">{{ $item->item->name }}</span>
                                <span class="text-gray-500"> × {{ $item->quantity }}</span>
                                @if($item->foc_quantity > 0)
                                <span class="text-green-600 text-xs myanmar-text"> (FOC: {{ $item->foc_quantity }})</span>
                                @endif
                            </div>
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div class="flex items-center">
                            ...
                            <label for="applyTax" class="ml-3">
                                ...
                                <span class="text-sm text-gray-500 ml-2">({{ \App\Models\Setting::get('tax_percentage', 0) }}%)</span>
                            </label>
                        </div>
                        @if($applyTax)
                        <span class="font-semibold text-gray-900">{{ number_format($calculatedTax, 0) }} Ks</span>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div class="flex items-center">
                            ...
                            <label for="applyService" class="ml-3">
                                ...
                                <span class="text-sm text-gray-500 ml-2">({{ number_format(\App\Models\Setting::get('service_charge', 0), 0) }} Ks)</span>
                            </label>
                        </div>
                        @if($applyService)
                        <span class="font-semibold text-gray-900">{{ number_format($calculatedService, 0) }} Ks</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-3 gap-3 mb-3">
                        <button 
                            ...
                            class="px-4 py-3 rounded-lg font-medium flex flex-col items-center {{ $paymentMethod === 'cash' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}"
                        >
                        ...
                        <button 
                            ...
                            class="px-4 py-3 rounded-lg font-medium flex flex-col items-center {{ $paymentMethod === 'card' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}"
                        >
                            <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button 
                            ...
                            class="px-4 py-3 rounded-lg font-medium flex flex-col items-center {{ $paymentMethod === 'mobile' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}"
                        >
                            <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-3 gap-3 mb-3">
                        <button 
                            ...
                            class="px-4 py-2 rounded-lg font-medium {{ $discountType === 'none' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}"
                        >
                            None
                        </button>
                        <button 
                            wire:click="$set('discountType', 'percentage')"
                            class="px-4 py-2 rounded-lg font-medium {{ $discountType === 'percentage' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}"
                        >
                            %
                        </button>
                        <button 
                            wire:click="$set('discountType', 'fixed')"
                            class="px-4 py-2 rounded-lg font-medium {{ $discountType === 'fixed' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}"
                        >
                            Fixed
                        </button>
                    </div>
                    ...
                    <div class="flex items-center space-x-2">
                        <input 
                            ...
                            placeholder="{{ $discountType === 'percentage' ? 'Percentage (%)' : 'Amount (Ks)' }}"
                        >
                        <span class="text-gray-600">{{ $discountType === 'percentage' ? '%' : 'Ks' }}</span>
                    </div>
                ...
                <div class="border-t pt-4 space-y-2">
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-medium">{{ number_format($calculatedSubtotal, 0) }} Ks</span>
                    </div>
                    @if($applyTax)
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600">Tax</span>
                        <span class="font-medium">{{ number_format($calculatedTax, 0) }} Ks</span>
                    </div>
                    @endif
                    @if($applyService)
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600 myanmar-text">ဝန်ဆောင်မှု</span>
                        <span class="font-medium">{{ number_format($calculatedService, 0) }} Ks</span>
                    </div>
                    @endif
                    @if($calculatedDiscount > 0)
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600 myanmar-text">လျှော့ဈေး</span>
                        <span class="font-medium text-green-600">-{{ number_format($calculatedDiscount, 0) }} Ks</span>
                    </div>
                    @endif
                    <div class="flex justify-between text-lg font-bold pt-2 border-t">
                        <span class="myanmar-text">စုစုပေါင်း</span>
                        <span class="text-green-600">{{ number_format($calculatedTotal, 0) }} Ks</span>
                    </div>
                </div>
