        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
            @forelse($tables as $table)
                ...
                <button 
                    ...
                    class="relative bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-200 p-4 border-2 {{ $isOccupied ? 'border-red-200 bg-red-50' : ($table->status === 'reserved' ? 'border-yellow-200 bg-yellow-50' : 'border-green-200 hover:border-green-300') }}"
                >
                    <!-- Status Indicator -->
                    <div class="absolute top-2 right-2">
                        <div class="w-3 h-3 rounded-full {{ $isOccupied ? 'bg-red-500 animate-pulse' : ($table->status === 'reserved' ? 'bg-yellow-500' : 'bg-green-500') }}"></div>
                    </div>

                    <!-- Table Icon -->
                    <div class="mb-2">
                        <svg class="w-10 h-10 sm:w-12 sm:h-12 mx-auto {{ $isOccupied ? 'text-red-600' : ($table->status === 'reserved' ? 'text-yellow-600' : 'text-green-600') }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
